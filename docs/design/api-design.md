...
建议至少保留以下四类核心数据：User、Entry、EmotionAnalysis、Diary、Report。它们分别承载用户身份、原始输入、AI 分析结果、整理后的日记以及日报、周报或月报。
...
第一阶段优先完成注册、登录、当前用户信息、快速创建并分析 Entry、日记的增查改删、首页统计、情绪趋势、情绪分布，以及管理员用户列表和管理员统计接口。
...
前端请求层建议按业务拆分为 client.ts、auth.ts、users.ts、entries.ts、speech.ts、diaries.ts、analyses.ts、stats.ts、reports.ts、conversations.ts、files.ts 和 admin.ts。
